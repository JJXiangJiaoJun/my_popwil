...
//---------------------------------------------------------------------------------------------
    ...
    QVBoxLayout *ChartVBoxLayout = new QVBoxLayout(parent);
//------------------------------实时曲线绘制UI----LeftFrame---------------------------------------------------------------
    m_ChartViewer=new QChartViewer((QWidget *)parent);
    m_ChartViewer->setGeometry(10, 70, 640, 480);
    ...
    m_ViewPortControl->setGeometry(10, 520, 640, 80);
    m_ViewPortControl->setViewer(m_ChartViewer);
    ChartVBoxLayout->addWidget(m_ChartViewer);
    //ChartVBoxLayout->addStretch(5);
    ChartVBoxLayout->addWidget(m_ViewPortControl);
    //this->setLayout(ChartVBoxLayout);
//---------------------------------参数初始化------------------------------------------------------------
...
void MyChartViewer::start(){
    m_ChartUpdateTimer->start(PAINT_PERIOD);//绘图定时器开启
}
...
void MyChartViewer::onChartUpdateTimer()
{
    ...
    int count;
    DataPacket *packets;
    if ((count = g_buffer.get(&packets)) <= 0)
        return;
